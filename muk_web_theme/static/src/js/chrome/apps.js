...
AppsMenu.include(_.extend({}, MenuSearchMixin, {
    ...
    init(parent, menuData) {
        ...
        for (let n in this._apps) {
            ...
            this._apps[n].fa_icon = menuData.children[n].fa_icon;
            this._apps[n].css_class = menuData.children[n].css_class;
        }
...
//    },
 _onMenuHide(event) {
        var res = $('.oe_wait').length === 0 && !this.$('input').is(':focus');
        var $dropdown_menu = $('.o_menu_apps > .dropdown > .dropdown-menu');
        if (res === false){
            setTimeout(function () {
