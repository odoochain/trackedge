...
fields.FieldOne2Many.include({
    _renderButtons() {
        ...
        if (config.device.isMobile && this.$buttons) {
        	...
        	const $buttons2 = $buttons.clone();
        	$buttons.addClass('btn-primary mk_mobile_add');
            $buttons.removeClass('btn-secondary');
            const $barcode = $('[name="open_picking_client_action_form"]');
            if ($barcode.length){
                const $barcodebtn = $('<button type="button" class="btn o-kanban-button-new btn-primary mk_mobile_add" accesskey="c"><i class="d-inline d-sm-none fa fa-barcode" title="Barcode"></i><span class="d-none d-sm-inline">Add</span></button>');
                $buttons.after($barcodebtn);
