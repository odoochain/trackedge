...
class Builder(models.Model):
    ...
    portal = fields.Boolean("Portal", tracking=True, help="Form is accessible by assigned portal user", default=True)
    ...
    public = fields.Boolean("Public", tracking=True, default=True,
    ...
    pub_url = fields.Char(string="Public URL ERP")
    helpdesk = fields.Boolean(string="Helpdesk", default=False)
    for_both = fields.Boolean(string="Trackedge/Helpdesk", default= False)
    its = fields.Boolean(string="ITS", default=False)
    trackedge = fields.Boolean(string="Trackedge", default=False)
    ...
    def _compute_display_fields(self):
        ret = super(Builder, self)._compute_display_fields()
        # print(self.title)
        for rec in self:
            rec.display_name_full = rec.title
        return ret
    @api.model
    ...
    def _default_formio_version_id(self):
        ...
        if default_version:
            ...
            all_versions = self.env['formio.version'].search([])
            if len(all_versions):
                return all_versions[0].id
            return False
    ...
    def _compute_display_fields(self):
        for r in self:
            ...
            if self._context.get('display_name_title'):
                ...
                r.display_name_full = _("{title}").format(title=r.title)
