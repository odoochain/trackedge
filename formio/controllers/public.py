...
class FormioPublicController(http.Controller):
    ...
    def public_form_create_submit(self, builder_uuid, **post):
        ...
        if 'instance' in post:
            try:
                ins = post['instance'].split('?')
                instance_a = str(ins[1])
                instance_b = instance_a.split('=')[1]
                email_a = str(ins[2])
                email_b = email_a.split('=')[1]
                company_a = str(ins[2])
                company_b = company_a.split('=')[1]
                for x in ins:
                    a = x.split('=')
                    print(a)
                vals['submission_instance'] = instance_b
                vals['submission_email'] = email_b
                vals['submission_company'] = company_b
                if 'data' in post:
                    if 'urgency' in post['data']:
                        vals['urgency'] = post['data']['urgency']
                    if 'impact' in post['data']:
                        vals['impact'] = post['data']['impact']
                # vals['submission_email'] = post['instance'].split('=')[2]
                # vals['submission_company'] = post['instance'].split('=')[3]
