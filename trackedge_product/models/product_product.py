...
fields_to_hide_in_search = ['description_purchase', 'description_sale', 'sequence', 'description_purchase',
                            ...
                            'life_time', 'use_time', 'removal_time', 'alert_time', 'spares_criticality',
...
class ProductProduct(models.Model):
    ...
    system_type_ids = fields.Many2many('system.type', string='System Types')
    technology_type_ids = fields.Many2many(
        'technology.type',
        'item_technology_type_relation',
        'item_id',
        'type_id',
        string='Technology Types')
    frequency_ids = fields.Many2many('item.frequency', string='Frequencies')
    ...
    repair_price = fields.Float('Repair Price',
        digits='Product Price')
    aftermarket_buy_price = fields.Float('Aftermarket Buy Price',
        digits='Product Price')
    aftermarket_replace_price = fields.Float('Aftermarket Replace Cost',
        digits='Product Price')
    ...
    screen_id = fields.Char("Screen ID")
    test_id = fields.Char("Test ID")
    repair_id = fields.Char("Repair ID")
    screening_documentation = fields.Char("Screening Documentation")
    test_documentation = fields.Char("Test Documentation")
    repair_documentation = fields.Char("Repair Documentation")
    ...
    def check_erp_code(self):
        ...
        if self.erp_code:
            ...
            code = self.env['product.product'].search(domain)
            if code:
                raise UserError(
    ...
    def create(self, vals):
        # User can only create consumables the rest he/she need access rights
    ...
    def get_image_url(self, s3Client=False, skip_already_updated=False):
        IrDefault = self.env['ir.default'].sudo()
        upload_bucket = IrDefault.get(
            'res.config.settings', "aws_upload_bucket")
        for record in self:
            if record.part_image_url and record.part_image_url != '' and \
                    skip_already_updated == True:
                print("Skipping part image update.....: %s" % record.default_code)
                continue
            if s3Client:
