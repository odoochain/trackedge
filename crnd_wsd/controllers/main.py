...
def get_redirect():
    ...
    'http://localhost:11069/helpdesk/new/step/category?service_id=1'
...
def guard_access(func):
    ...
    >>>@http.route(["/helpdesk/new"], type='http', auth="public",
...
class WebsiteRequest(WSDControllerMixin, http.Controller):
    ...
    @http.route(['/helpdesk',
                 '/helpdesk/<string:req_status>',
                 '/helpdesk/<string:req_status>/page/<int:page>'],
    ...
    @http.route(["/helpdesk/request/<int:req_id>"],
                type='http', auth="public", website=True)
    ...
    def request_new(self, **kwargs):
        ...
        return http.request.redirect(QueryURL(
            '/helpdesk/new/step/category', [])(**kwargs))
    ...
    @http.route(["/helpdesk/new/step/category"], type='http', auth="public",
    ...
    @http.route(["/helpdesk/new/step/type"], type='http', auth="public",
                methods=['GET', 'POST'], website=True)
    @guard_access
    def request_new_select_type(self, type_id=None, category_id=None,
        ...
        if http.request.httprequest.method == 'POST' and req_type:
            return http.request.redirect(keep(
                '/helpdesk/new/step/data', type_id=req_type.id,
                category_id=req_category.id, **kwargs))
        ...
        if len(public_types) == 1 and not http.request.session.debug:
            return http.request.redirect(keep(
                '/helpdesk/new/step/data', type_id=public_types.id,
                category_id=req_category.id, **kwargs))
    ...
    @http.route(["/helpdesk/new/step/data"],
            ...
            if not validation_errors:
                ...
                try:
                    ...
                    if redirect_after_request_created == 'req_page':
                        redirect_url = "/helpdesk/request/%s" % req.id
                        redirect_params['show_congrats_msg'] = True
                    else:
                        redirect_url = "/helpdesk/congrats/%s" % req.id
    ...
    @http.route(["/helpdesk/congrats/<int:req_id>"],
                type='http', auth="public", website=True)
